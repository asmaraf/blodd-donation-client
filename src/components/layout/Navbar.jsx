...
import { Droplet, LayoutDashboard, LogOut, HeartHandshake, FileText, Menu, X, ChevronDown } from 'lucide-react';
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          {!user ? (
            <div className="flex items-center gap-3 ml-2">
              <Link
                to="/login"
                className="px-4 py-2 text-sm font-semibold text-slate-200 bg-slate-800/80 hover:bg-slate-700/80 rounded-xl transition border border-slate-700/60"
              >
                Login
              </Link>
              <Link
                to="/register"
                className="px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-rose-600 to-rose-700 hover:from-rose-500 hover:to-rose-600 rounded-xl shadow-lg shadow-rose-950/50 transition"
              >
                Join as Donor
              </Link>
            </div>
          ...
          {user ? (
            ...
            <div className="flex flex-col gap-2 pt-2">
              <Link
                to="/login"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-2.5 text-sm font-semibold text-slate-200 bg-slate-800 rounded-xl"
              >
                Login
              </Link>
              <Link
                to="/register"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-2.5 text-sm font-semibold text-white bg-rose-600 rounded-xl"
              >
                Join as Donor
              </Link>
            </div>
